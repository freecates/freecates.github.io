...
{       // Event simulator
                ...
                case 'checkbox':
                    if(numeric) {
                        ...
                        $$("#id_" + data + ' input[type="checkbox"]').each(function(chk) {
                            if(chk.checked) { 
                                val += parseFloat(chk.value.replace(/-?([^0-9])/g,"$1").replace(/[^0-9\.-]/g,"")) || 0;
                            }
                        });
                    } else {
                        var valArr = [];
                ...
                if(character === '[' && !numeric) {
                    ...
                    try {
                        ...
                        if(equation.charAt(i+1) === '(') {
                            i+=2;
                            var endSpecial = equation.indexOf(')', i);
                            if(endSpecial === -1) continue;
                            var args = equation.substring(i, endSpecial);
                            args = args.split(',');
                            for(var j=0;j<args.length;j++) {
                                args[j] = parseFloat(calculate(args[j], true));
                            }
    ...
    countTotal: function(prices){
        ...
        $H(prices).each(function(pair){
            ...
            if ($(pair.key).checked) {
                ...
                if (pair.value.shipping && pair.value.shipping.firstItem) {
                    ...
                    if (qty === 1){
                        cost = parseInt(shipping.firstItem);
                    } 
                    if (qty > 1 ) {
                        cost = !shipping.addItem ? parseInt(shipping.firstItem) : parseInt(shipping.firstItem) + parseInt(shipping.addItem) * (qty -1);
                    }
                }
                shippingCost += cost;
                subTotal += price;
        ...
        if ($("payment_shipping")){
            $("payment_shipping").update(parseFloat(shippingCost).toFixed(2));
        }
